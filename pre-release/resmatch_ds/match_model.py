...
def dual_softmax(p,dustbin=None):
    p = p.clamp(max=50)
    b,m,n = p.shape
    if dustbin is not None:
        bins0 = dustbin.expand(b, m, 1)
        bins1 = dustbin.expand(b, 1, n)
        p_row = (torch.softmax(torch.cat([p,bins0],dim=-1),dim=-1)+eps).log()+math.log(p.shape[-1]+1)
        p_col = (torch.softmax(torch.cat([p,bins1],dim=-2),dim=-2)+eps).log()+math.log(p.shape[-2]+1)
        return p_row[:,:,:-1]+p_col[:,:-1,:]
    else:
        return (torch.softmax(p,dim=-1)+eps).log()+(torch.softmax(p,dim=-2)+eps).log()+math.log(p.shape[-1])+math.log(p.shape[-2])
...
class matcher(nn.Module):
    ...
    def forward(self,data,test_mode=True):
        ...
        cross_simi = torch.einsum('bcn,bcm->bnm',desc1_,desc2_)
        cross_res = dual_softmax(cross_simi)
        ...
        for i in range(self.layer_num):
            if i in self.adjlayer and i!= 0:
                ...
                cross_simi = (torch.einsum('bcn,bcm->bnm',aug_desc1_desc,aug_desc2_desc)+cross_simi)/2
                cross_res = dual_softmax(cross_simi)
